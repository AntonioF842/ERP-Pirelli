...
class ProductionRecipeDetailView(QDialog):
    ...
    def init_ui(self):
        ...
        self.setWindowTitle(f"Receta: {self.recipe_data.get('id_receta', 'Detalle')}")
        ...
        # Icono y Título
        icon_label = QLabel()
        icon_pixmap = QPixmap("resources/icons/recipe.png")
        ...
        # Conectar eventos
        edit_button.clicked.connect(lambda: self.edit_requested.emit(self.recipe_data))
        delete_button.clicked.connect(self.confirm_delete)
        close_button.clicked.connect(self.close)
        
        # Información de la receta
        info_group = QGroupBox("Detalles de la Receta")
        info_layout = QGridLayout()
        
        # Producto
        info_layout.addWidget(QLabel("Producto:"), 0, 0)
        producto_label = QLabel(self.recipe_data.get("producto_nombre", "N/A"))
        ...
        material_label = QLabel(self.recipe_data.get("material_nombre", "N/A"))
        ...
        cantidad_label = QLabel(f"{float(self.recipe_data.get('cantidad', 0)):.2f}")
    ...
    def confirm_delete(self):
        ...
        reply = QMessageBox.question(
            ...
            f"¿Está seguro que desea eliminar esta receta?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No
        )
        
        if reply == QMessageBox.StandardButton.Yes:
            # Emitir señal con el ID de la receta
