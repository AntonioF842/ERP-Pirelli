...
class ProductDetailView(QDialog):
    ...
    def init_ui(self):
        ...
        self.setWindowTitle(f"Producto: {self.product_data.get('nombre', 'Detalle')}")
        ...
        # Icono y Título
        icon_label = QLabel()
        icon_pixmap = QPixmap("resources/icons/product.png")
        if not icon_pixmap.isNull():
            icon_label.setPixmap(icon_pixmap.scaled(64, 64, Qt.AspectRatioMode.KeepAspectRatio))
        
        title_label = QLabel(self.product_data.get("nombre", "Producto"))
        ...
        # Conectar eventos
        edit_button.clicked.connect(lambda: self.edit_requested.emit(self.product_data))
        ...
        # Código
        info_layout.addWidget(QLabel("Código:"), 0, 0)
        codigo_label = QLabel(self.product_data.get("codigo", "N/A"))
        codigo_label.setStyleSheet("font-weight: bold;")
        info_layout.addWidget(codigo_label, 0, 1)
        
        # Categoría
        info_layout.addWidget(QLabel("Categoría:"), 0, 2)
        categoria_label = QLabel(self.product_data.get("categoria", "N/A"))
        categoria_label.setStyleSheet("font-weight: bold;")
        info_layout.addWidget(categoria_label, 0, 3)
        
        # Precio
        info_layout.addWidget(QLabel("Precio:"), 1, 0)
        precio_label = QLabel(f"${float(self.product_data.get('precio', 0)):.2f}")
        ...
        descripcion_label = QLabel(self.product_data.get("descripcion", "Sin descripción"))
    ...
    def confirm_delete(self):
        """Solicita confirmación para eliminar el producto"""
        ...
        if reply == QMessageBox.StandardButton.Yes:
            # Emitir señal con el ID del producto
